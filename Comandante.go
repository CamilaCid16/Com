...
func Consultar(m pb.KaisClient) {
	...
	responseS, err := s.GetEnemigosServidor(context.Background(), &pb.Direccion{Dir: ""})

	if err != nil {
		log.Println("ERROR")
	}
	
	str := strconv.FormatInt(responseS.Cantidad, 10)
	fmt.Println("Enemigos encontrados: " + str)
}
